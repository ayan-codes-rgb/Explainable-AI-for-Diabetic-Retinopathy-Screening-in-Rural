...
% Keep the raw scores so rocReport can draw the full trade-off curve without
% re-extracting features.
scores = struct( ...
    'positiveClass', posName, ...
    'val',  struct('score', Pval(:,posCol),  'truth', yVal), ...
    'test', struct('score', Ptest(:,posCol), 'truth', yTest));
